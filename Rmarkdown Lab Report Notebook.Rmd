...
author: Lina Hohlbein, Mira Kracke, Lukas Zimmermann
...
We also wanted to visualize this , so next we made two graphs to show both the percental change, as well as the total amount that had changed as compared to the value of the previous year.
```{r}
...
At this point it should be noted that the filter function did not work for all data sets as a whole. The reason for this was not obvious, since the application was the same for each of the five data sets. 

```{r}
ggplot(data = `df_Total eutrophication`, aes(x=Year, y=Total)) + 
  geom_bar(stat="identity", width=0.9, fill="blueviolet") +
  theme_minimal() +
...
```{r}
ggplot(data = `df_Quality class II-III`, aes(x=Year, y=QC)) + 
  geom_bar(stat="identity", width=0.7, fill="cadetblue1") +
  theme_minimal() +
  ggtitle("Development of water quality class II-III in Germany from 1982 - 2018") +
  xlab("Year") +
  ylab("Eutrophication (%)")
```
The development in water quality class II-III indicates an increasing development of eutrophication until 2015. From 2015 the eutrophication decreases slightly, but the value is still over 50% in 2018, so the target of 0.00% eutrophication by 2030 seems far away.

```{r}
ggplot(data = `df_Quality class III`, aes(x=Year, y=QC)) + 
  geom_bar(stat="identity", width=0.7, fill="cadetblue2") +
  theme_minimal() +
  ggtitle("Development of water quality class III in Germany from 1982 - 2018") +
  xlab("Year") +
  ylab("Eutrophication (%)")

```
Water quality class III is considered in the third presentation. The development shows the highest level of eutrophication in 1993. From this time on, the trend is downward, but always with increasing outbreaks of eutrophication between the different years. In the last measured year, however, the eutrophication is below five percent. 

```{r}
ggplot(data = `df_Quality class III-IV`, aes(x=Year, y=QC)) + 
  geom_bar(stat="identity", width=0.7, fill="cadetblue3") +
  theme_minimal() +
  ggtitle("Development of water quality class III-IV in Germany from 1982 - 2018") +
  xlab("Year") +
  ylab("Eutrophication (%)")
...
```{r}
ggplot(data = `df_Quality class IV`, aes(x=Year, y=QC)) + 
  geom_bar(stat="identity", width=0.7, fill="cadetblue4") +
  theme_minimal() +
  ggtitle("Development of water quality class IV in Germany from 1982 - 2018") +
  xlab("Year") +
  ylab("Eutrophication (%)")
...
In order to be able to look at the individual developments in more detail, the data will be presented in more detail using ggoints and gglines.
...
`df_Total eutrophication` %>% 
  ...
  labs(x = "Time (years)", 
       y = "Eutrophication (%)", 
       title = "Development of total eutrophication in Germany 1982-2018") +
  xlim(1982, 2030) +
  ylim(0, 100)
```

The second graph of total eutrophication shows the development of eutrophication again in a clearer picture. However, the clear trend towards a reduction in eutrophication can also be seen here. 
...
By presenting the data in the other graphs, the developments of water quality classes III, III-IV and IV can be named with a clear positive trend towards attainment of 0.00% eutrophication in 2030.
...
```{r}
label_percent(scale = 1)(((71.2 / 90.1 * 100) - 100))
```
...
```{r}
label_percent(scale = 1)(((69.4 / 71.2 * 100) - 100))
```
The decrease in eutrophication was 21% from 1985 - 2000 and only 3% in the following 15 years. This results in an average decrease in eutrophication of 0.8% per year for this period of 30 years.  

```{r}
"Decreas per Year" <- (24/30)
view(`Decreas per Year`)
```

For the next 15 years, if eutrophication continues to decrease, this would mean another 12% decrease. Conversely, based on these data, this would mean that in 2030 the water bodies would still be up to 52% eutrophicated. So the target of 0.00% would have been missed.
...
ggplot() + geom_line(aes(x = Year, y = Perc_change.x), size = 0.8, color = "mediumaquamarine", data = df_perc_change) + 
  ...
  labs(title = "Percentage change 2001-2019", subtitle = "green = phosphate, orange = nitrogen, purple = eutrophication", x = "Year", y = "Percantage change") +
  scale_y_continuous(labels = scales::percent) +
...
```{r}
ggplot() + 
  geom_bar(aes(x = Year, y = Perc_change.x), stat="identity", width=0.7, fill="mediumaquamarine", position = "dodge", data = df_perc_change) +
  geom_bar(aes(x = Year, y = Perc_change.y), stat="identity", width=0.7, fill="coral", position = "dodge", data = df_perc_change) +
  geom_bar(aes(x = Year, y = Perc_change), stat="identity", width=0.7, fill="blueviolet", position = "dodge", data = df_perc_change) +
  theme_minimal()
```
## **Discussion**
...
BMU 2016. Chemische Gewässergüteklassen. Abgerufen am 21. Dezember, 2020, von https://www.bmu.de/themen/wasser-abfall-boden/binnengewaesser/fluesse-und-seen/schutzziele-und-bewertungsparameter/chemische-gewaessergueteklassifizierung/
