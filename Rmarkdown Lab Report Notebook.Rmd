...
In the interest of nature conservation our interest is to see in which way the development of eutrophication of ecosystems and the usage of nitrogen and phosphate fertilizers in Germany are interrelated. Despite regulations on the usage of too much fertilizers than necessary the disbalance of nutrients and their functions in aquatic ecosystems are still visible and take long to recover. 
Therefore it is difficult too say if regulations on the usage of fertilzers have an direct impact on ecosystems. To see both the development of eutrophication levels and the amount of fertilizer used in Germany we took accessible data from the Federal Environment Agency and Eurostat. We looked at phosphate and nitrogen usage in agriculture and the development of different water quality classes. To get an overview we transfered all our data in one table in R, and then looked at summary data of each section, meaning max, min, mean and median values. For each dataset we built barplot and line graphs to see the development over the years and further looked on years with striking values compared by previos years, by looking at percentage and total change from year to year. Lastly we compare the development of our datasets in percentage change in one graph to discuss if the development interrelates or if similar trends can be found. In the datasets of the Federal Environment Agency targets for the year 2030 are presented which is determined by the sustainability strategy from 2016 (Umweltbundesamt, 2020). With that we finally try to make a statement if this goal can be reached based on the data we analyzed. The interpretation of the data does not include other factors like the current development and discussions on environmental and agricultural policies. The focus lays on the interpretation of the chosen data only.
...
as in 2009 and an increase again from 85 kg in 2014 to 103 kg in 2018. A general tendency of a decreasing use of nitrogen
...
consumption decreases continuasly to 2019, from 103 kg to 89 kg nitrogen per hectar.
...
Since all three datasets come with observations in different types of measurements and format, we decided to compare all three of them regarding their percentage change over the years, to bring them all on the same scale. For that we first created a new table which contained the percentage change columns of phosphate, nitrogen and eutrophication.
```{r}
...
ggplot() + geom_line(aes(x = Year, y = Perc_change.x), size = 0.8, color = "mediumaquamarine", data = df_perc_change) + 
  ...
  geom_line(aes(x = Year, y = Perc_change), size = 0.8, color = "blueviolet", data = df_perc_change) +
  labs(title = "Percentage change 2000-2019", subtitle = "green = phosphate, orange = nitrogen, purple = eutrophication", x = "Year", y = "Percantage change") +
  scale_y_continuous(labels = scales::percent) +
  scale_x_continuous(breaks=seq(2001,2019,2)) +
  theme_minimal()
```
The comparative line graph illustrates that phosphate consumption went through the biggest changes over the period from 2000-2019, while changes in nitrogen balance change a bit less, but often seem to mimic the changes in phosphate use with a 1-2 year time delay. Meanwhile, eutrophication only fluctuates moderately in comparison. This makes sense, since ecosystems such as the river sites where probes for the eutrophication value were observed, take longer to react to environmental changes such as the annual ups and downs of fertilizer use.
...
ggplot(comb2, aes(Year, Value, fill = group)) + 
  ...
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual("Type", values = c("Phosphate" = "mediumaquamarine", "Nitrogen" = "coral", "Eutrophication" = "blueviolet")) +
  labs(x = "Time (years)", 
       y = "Change (in percent)", 
       title = "Percentage change")
```
Again it can be seen that phosphate consumption fluctuated the most, while the nitrogen balance had the most dramatic changes between the years 2008 and 2015 and eutrophication goes back and forth ever so slightly with an increasing decrease over the last three years.


## **Discussion**

The data shows a trend of reduction of phosphate and nitrogen inputs into the ecosystem of water bodies. This in turn has a positive effect on the eutrophication in water bodies and their condition. With the exclusive consideration of these data the analysis comes to the conclusion that a decrease until 2030 of the phosphate values up to 0.0 % is possible. For nitrogen inputs, a reduction of 37.5% is possible according to the current approach. However, it was already mentioned at the beginning of the work that not only agriculture is involved in the input of nitrogen and phosphate, but also other carriers such as industry and municipal wastewater treatment plants. In recent years, the latter have significantly reduced their nutrient inputs to water bodies, whereas agriculture has remained relatively constant. (Taube et al. 2020. P. 7f.) The improvement in water quality could therefore also be attributed to the reduction in municipal wastewater treatment plants and less to the measures in agriculture. 
Reduction in agriculture is an acute problem. In order to protect ecosystems and not violate the EU nitrogen Directive. The European Union has already reminded Germany several times to implement the nitrogen Directive and reduce the input from agriculture (European Commission. 2019). 
...
Since only working with already collected data and a dataset which is limited to the annual overall use of phosphate and nitrogen in agriculture over a period of time, as well as a set of measurements on eutrophication in river ecosystems, this report can make no claim on providing a complete analysis of the topic and other factors which were not accounted for in our datasets may be at play. 
