...
Therefore it is difficult too say if regulations on the usage of fertilzers have an direct impact on ecosystems. But to see both the development of eutrophication levels and the amount of fertilizer used in Germany we took accessible data from the Federal Environment Agency and Eurostat. We looked at Phosphate and Nitrogen usage in agriculture, the development of different water quality classes. To get an overview we transfered all our data in one table in R, and then looked at summary data of each section, meaning max, min, mean and median values. For each dataset we built barplot and line graphs to see the development over the years and further looked on years with striking values compared by previos years, by looking at percentage and total change from year to year. Lastly we compare the development of our datasets in percentage change in one graph to see if the development interrelates or if similar trends can be found. In the datasets of the Federal Environment Agency targets for the year 2030 are presented which is determined by the sustainability strategy from 2016. With that we finally try to make a statement if it is realistic based on the data to reach that goal.
